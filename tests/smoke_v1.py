...
def test_schema_migration_metadata():
    with tempfile.TemporaryDirectory() as td:
        ...
        assert_true(user_version == 6, f'expected schema user_version=6, got {user_version}')
        row = con.execute('SELECT schema_checksum FROM schema_migrations WHERE schema_version=?', (6,)).fetchone()
